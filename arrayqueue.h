...
template<typename T> ArrayQueue<T>::~ArrayQueue() { delete[] elements_; }

template<typename T> bool ArrayQueue<T>::isEmpty() const { return size() == 0; }

template<typename T> void ArrayQueue<T>::clear() { size_ = 0; }

template<typename T> T& ArrayQueue<T>::front() { return elementAt(0); }

template<typename T> void ArrayQueue<T>::enqueue(const T& value) {
  if (size() >= capacity_) { expand(); }
...
template<typename T> void ArrayQueue<T>::dequeue() {
  assert(!isEmpty());
...
template<typename T> ArrayQueue<T>::ArrayQueue(size_t capacity)
  : capacity_(capacity), elements_(new T[capacity_]), beginIndex_(0), size_(0) {}
...
template<typename T> size_t ArrayQueue<T>::size() const { return size_; }
...
template<typename T> void ArrayQueue<T>::expand() {
  ...
  for (size_t index = 0; index < size(); index++) { newObject.enqueue(elementAt(index)); }
  swap(newObject);
}
